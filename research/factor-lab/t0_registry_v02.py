...
PIT date cap, V01->V02 population diff, and repaired-date impact.
...
V02 input authority:
  ...
  DAILY_SHA256 (expected) = 75fa706dd8575ee5c43d4a9462adda39c8e17af4c72866b00b1fabacea364dc0

Population-blocker note: V01's 22393 was revoked because 12 July sessions
(07-09..07-24) were PROVISIONAL (no TUSHARE/ASL consensus). V02 rebuilds on
the repaired snapshot where those 12 sessions are CONFIRMED. A different
population is EXPECTED, not a failure.
"""
...
def load_pool_v02(path: Path) -> tuple[dict[str, tuple], str]:
    actual = sha256(path)
    return _load_pool_by_code(path), actual
...
def population_accounting_v02(
    ...
    """V02 accounting: episode mismatch is REPORTED, not fatal (the repaired
    timeline legitimately changes the population)."""
...
def render_report(a: dict) -> str:
    ...
    lines = [
        ...
        f"- OBSERVED_SIGNAL_N = {a['OBSERVED_SIGNAL_SETUP_N']}；NEVER_SIGNAL_N = {a['NEVER_SIGNAL_SETUP_N']}",
...
def main() -> dict:
    ...
    # 2. input provenance gate
    daily = load_daily_v02(DAILY_PATH, DAILY_SHA256_EXPECTED)
    pool, pool_sha = load_pool_v02(POOL_PATH)
    ...
    # 4. parity gate on the deterministic bounded sample (frozen semantics)
    ...
    # 5. full anchor-only registry (frozen V01 scan, new input)
    ...
    # 6. population accounting (V02: mismatch reported, not fatal)
    acct = population_accounting_v02(rows, episodes)

    # 7. V01 vs V02 diff + repaired-date impact
    diff = v01_vs_v02_diff(rows)

    # 8. July session visibility on the new snapshot
    july_days = sorted(
        {
            b.trade_date
    ...
    out = {
        ...
        **pit,
        "JULY_SESSIONS_2026": len(july_days),
        "PARITY": parity,
        **acct,
        "DIFF": diff,
