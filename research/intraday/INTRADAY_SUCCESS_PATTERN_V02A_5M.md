...
B. FAST_RECLAIM = NOT_MEASURED / NO_CLEAR_EVIDENCE。
   VWAP_FIRST_RECLAIMED 只表示“checkpoint 前发生过一次 BELOW→ABOVE VWAP
   reclaim”，不测量 recovery duration / speed，因此不得解释为“收复更快”。
   描述性事实（VWAP_RECLAIM_OCCURRENCE_BY_CHECKPOINT）：允许首根 bar 参与后，
   09:45 S 27.5% vs F 23.2%；10:30 S 57.5% vs F 47.5%（+0.100）；
   11:30 S 75% vs F 59.6%（+0.154）——存在比例差异，但这是 occurrence 差异，
   不是速度证据。OPEN_RECLAIMED_NOW 早盘接近；VWAP_SECOND_RECLAIMED 无差异。
...
TOP_DESCRIPTIVE_PATTERN = S1_POSITION_PLUS_VWAP_STRENGTH_BY_1030
（研究总结：10:30-11:30 收盘位于 S1 上方 + 正偏离 VWAP 扩大 + 累计收益更强；
DIST_TO_VWAP d=0.50/0.59、DIST_TO_S1 d=0.27/0.43、S1_STATE rate diff
+0.23/+0.17 方向一致。）

明确：这是多个独立 descriptive feature 方向一致后的研究总结，不是已验证的
composite signal；本轮未计算 S1_STATE==ABOVE AND VWAP_STATE==ABOVE 的联合
统计，也不新增 joint metric（避免 post-hoc feature creation）。
...
TIME_OF_MORNING_LOW（中位相同）、
VWAP_SECOND_RECLAIMED、S1_SECOND_RECLAIMED（计数无差异）、LOW_PROGRESSION。

NEW_LOW_AFTER_OPEN_RECLAIM（修正口径）：11:30 SUCCESS 5% vs FAILED_BREAKOUT
11.1%（方向 F 更多）——WEAK / NO_CLEAR_SIGNAL。
...
| TOP_SIGNAL | S1_AND_VWAP_HOLD_BY_1030（旧命名） | TOP_DESCRIPTIVE_PATTERN = S1_POSITION_PLUS_VWAP_STRENGTH_BY_1030（多个独立 feature 方向一致的总结，非已验证 composite；未计算联合统计） | CHANGED（命名/语义） |
| B FAST_RECLAIM | 无强区别（旧叙述） | NOT_MEASURED / NO_CLEAR_EVIDENCE；VWAP_RECLAIM_OCCURRENCE_BY_CHECKPOINT 10:30/11:30 有比例差异（+0.100/+0.154），不代表速度 | CHANGED（语义） |
...
FAST_RECLAIM（由“有/无区别”叙述改为 NOT_MEASURED：仅 VWAP reclaim occurrence
比例差异，不测量速度）；
...
未改变结论：HYPOTHESIS=NOT_SUPPORTED、TOP_DESCRIPTIVE_PATTERN 方向、
OPENING_DRAWDOWN 无差异、SECOND_RECLAIM 计数无差异、VWAP_ACCEPTANCE 方向。

## CORE CONCLUSION LABELS

| 项目 | 标签 |
|---|---|
| OPENING_DRAWDOWN | NO_CLEAR_SIGNAL |
| FIRST_S1_TOUCH（alone） | NO_SIGNAL |
| S1_TOUCH_REJECTED_IMMEDIATELY | DESCRIPTIVE_FAILURE_LEAN |
| S1_ACCEPTED_THEN_REBREAK | DESCRIPTIVE_FAILURE_LEAN |
| VWAP_POSITION / DIST_TO_VWAP | DESCRIPTIVE_DIFFERENCE_FROM_1000 |
| HIGH_PROGRESSION | SECONDARY_DESCRIPTIVE_DIFFERENCE |

全部：OBSERVE_ONLY / HUMAN_HEURISTIC_CANDIDATE，不得 promotion。
