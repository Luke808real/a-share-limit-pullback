...
def b2_volume_vs_20d_mean(bars, anchor_date: date, b2_date: date) -> Decimal | None:
    ...
    Contract (FACTOR_CATALOG F20): PRE20 = strictly the last 20 visible
    trading sessions before B2 (trade_date < b2_date only; future rows never
    participate). B2 itself is never part of the mean. None when fewer than
    20 pre-B2 sessions exist (PRE20_N < 20 -> None) or when the window mean
    volume is zero. Fail closed like F19: anchor or B2 bar missing ->
    ValueError.
    """
    ordered = _ordered(bars)
    _require_anchor(ordered, anchor_date)
