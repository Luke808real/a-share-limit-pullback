    ...
    *,
    ...
    if reader == "query":
        ...
        with WarehouseMetadata(layout.duckdb_path) as metadata:
            # SHADOW: base create_snapshot accepts materialized daily_rows
            # (the streaming daily_row_chunks variant lives on the phase1
            # branch); materialize bounded chunks for this dry run.
            daily_rows = [row for chunk in _row_chunks() for row in chunk]
