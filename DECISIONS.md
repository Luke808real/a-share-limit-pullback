...
当前实现至阶段2C.1：阶段1.5纯策略计算、阶段2A固定真实数据适配器及单股票
`inspect`、阶段2B单股票逐日内存`replay`、风险快照时序、逐setup摘要，以及
S1压力审计、新建仓剩余空间、任意合法沪深主板单股票入口和Provider边界加固。
...
阶段2A时，`inspect` 仅支持 002606、603123、001382 的单股票、单评价日内存计算。
该临时白名单已由D-025取代。涨停池
...
单股票入口不再维护001382、002606、603123白名单。唯一共享代码解析器只接受
六位ASCII数字字符串，并把深圳000/001/002/003及上海600/601/603/605映射为
MAIN、交易所和BaoStock代码；创业板、科创板、B股、北交所及未知前缀在Provider
调用前拒绝。格式及板块合法只代表可以查询，不保证指定日期存在行情。

`inspect`固定为`STATELESS_INSPECT`，不接收或伪造`previous_signal`，不能用来
还原B2生命周期。`replay`固定为`POINT_IN_TIME_REPLAY`，逐交易日传入上一完整
信号，仍是验证历史状态和快照的正式入口。两者都只处理用户明确指定的一个代码；
解除白名单不代表建立股票池或开放全市场扫描。

BaoStock返回前按`(code, trade_date)`确定性检查重复：完全相同的重复行保留一条、
质量至少PARTIAL并记录flag，字段冲突则显式失败。必填字段异常、可选字段缺失和
Provider会话失败均在边界层保留具体原因；query失败后的logout失败不得覆盖原始
